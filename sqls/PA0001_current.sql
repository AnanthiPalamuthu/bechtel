-- negation logic
CREATE VIEW bbs_sap_pr.v_pa0001_ins_upd AS
SELECT pernr, begda, endda, concat(pernr,begda, endda) pbe FROM bbs_sap_pr.pa0001 
WHERE opr_ind in ('INS','UPD');
CREATE VIEW bbs_sap_pr.v_pa0001_del AS
SELECT pernr, begda, endda, concat(pernr,begda, endda) pbe FROM bbs_sap_pr.pa0001 
WHERE opr_ind in ('DEL');
drop table bbs_sap_mediate.pa0001;
create table bbs_sap_mediate.pa0001 stored as orc AS
select pa.* from bbs_sap_pr.pa0001 pa, 
(SELECT a.pernr, a.begda, a.endda
FROM bbs_sap_pr.v_pa0001_ins_upd A
LEFT OUTER JOIN bbs_sap_pr.v_pa0001_del B
ON (B.pbe = A.pbe)
...
-- latest of the pernr
drop table bbs_sap_mediate.pa0001_t;
create table bbs_sap_mediate.pa0001_t stored as ORC as 
select distinct * from (
	...
	from bbs_sap_mediate.pa0001 where (unix_timestamp (ENDDA, 'yyyyMMdd') > FROM_UNIXTIME(UNIX_TIMESTAMP()) or endda='99991231') and opr_ind != 'DEL'
	) t where rank = 1;


--final of the current table

drop table bbs_sap_hr_pr.pa0001;
create table bbs_sap_hr_pr.pa0001 stored as orc as
...
from bbs_sap_mediate.pa0001 p1, bbs_sap_mediate.pa0001_t p2
